...
export default function CategoryDetailPage() {
  ...
  /* Flatten all products when slug === "all" */
  type ProductWithCatSlug = (typeof menu.categories)[0]["products"][0] & { categorySlug: string };
  const allProducts: ProductWithCatSlug[] = isAll
    ? menu.categories.flatMap((c) => c.products.map((p) => ({ ...p, categorySlug: c.slug })))
    : (category!.products.map((p) => ({ ...p, categorySlug: categorySlug! })));

  const pageTitle = isAll ? tr.all ?? "Tüm Ürünler" : category!.name;
    ...
    <div className="luna-menu min-h-screen pb-24">
      ...
      <div className="max-w-xl mx-auto px-4 pt-4">
        ...
        <div className="flex items-center gap-1.5 text-xs mb-3">
          ...
          <span style={{ color: accent }}>{pageTitle}</span>
        </div>

        <h1 className="text-3xl font-bold text-white tracking-tight mb-4">{pageTitle}</h1>
